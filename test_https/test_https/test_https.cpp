// test_https.cpp : Defines the entry point for the console application.
//
...
/*
// harness_https.cpp : Defines the entry point for the console application.
//
...
*/    
    ...
    w=new tcp("127.0.0.1","443",ciphers);
    if(w)
    {
        //
        w->timeout(2,0);
	...
	printf("press any key\n"); _getch();
    w=new tcp("127.0.0.1","80",ciphers);
    if(w)
    {
        //
        w->timeout(2,0);
...
//		if(r) printf("RECV:\n%s\n\n",r);
    ...
    w=new tcp("msdn.pe.kr","80",ciphers);
    if(w)
    {
        //
        w->timeout(10,0);
